...
if __name__ == "__main__":
    ecg_path = "./HDF5_DATA/Code15/Code15_ECG.hdf5"
    image_dir = "./HDF5_DATA/Code15/images/"

    with h5py.File(ecg_path, "r") as file:
        ecgs = file['ECG'][()]
        sids = file['SID'][()]

    num_ids = len(sids)

    for id in tqdm.tqdm(range(num_ids), total=num_ids):
        plot(np.transpose(ecgs[id]), sample_rate=100, title = 'ECG', show_grid=True)
        save_as_png(f"sid-{sids[id]}", image_dir)
