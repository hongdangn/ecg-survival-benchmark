...
def Get_Global_String_List( time_h, time_m, GPU, mem, model_type, name_suffix_list = ['071725','2048Dim','ALL_DATASET']):
    ...
    # model_type: 'InceptionTimeReg', 'RibeiroReg'
    # name_chunks_list: list of details on the model to add to the name. Only used to identify it.
...
def decompress_args_list(args_list, Model_Type, Running_Arg = ''):
    ...
    # in next arg, only one option -> append it (it is a string) to Running_Arg (a string), then call self with one less arg
    ...
    # in next arg, multiple options -> call self, appending each option ( a stirng) to Running_Arg (a string)
    else:
...
for Model_Type in Model_Type_List:
    ...
    # args_list.append([ ' --Load Best'  ]) # If you want to load an existing model (best checkpoint)
    ...
    glob_args_list.append([ ' --censor 999'  ])
