...
args  = '--Train_Folder Code15'         # Training Data Folder Name
...
args += ' --GPU_minibatch_limit 64'          # how many samples go to GPU at one time (depends on GPU)
...
args += ' --debug False'                    # 'True' limits data to 5k samples of tr/test. At loading time.
...
args  = '--Train_Folder All'         # Training Data Folder Name
args += ' --Model_Name ZeroNet_DS_Debug'   # Must start with ModelType. InceptionTime or Ribeiro (ResNet) or ZeroNet (Returns a 0) or ECGTransForm
args += ' --Test_Folder Code15'         # Test Data Folder Name
args += ' --batch_size 512'                  # batch size
args += ' --epoch_end 10'                     # set to -1 if you don't want to train the model after initialization and loading
args += ' --validate_every 1'                # default; how often to get validation performance
# args += ' --Save_Out_Checkpoint True'        # default; are you saving out the checkpoint?
